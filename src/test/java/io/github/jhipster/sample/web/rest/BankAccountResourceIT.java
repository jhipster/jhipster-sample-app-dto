...
class BankAccountResourceIT {
    ...
    void createBankAccount() throws Exception {
        ...
        var returnedBankAccountDTO = om.readValue(
            restBankAccountMockMvc
                .perform(
                    post(ENTITY_API_URL).with(csrf()).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(bankAccountDTO))
                )
    ...
    void putWithMissingIdPathParamBankAccount() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restBankAccountMockMvc
            .perform(put(ENTITY_API_URL).with(csrf()).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(bankAccountDTO)))
    ...
    void patchWithMissingIdPathParamBankAccount() throws Exception {
        ...
        restBankAccountMockMvc
            .perform(
                patch(ENTITY_API_URL).with(csrf()).contentType("application/merge-patch+json").content(om.writeValueAsBytes(bankAccountDTO))
            )
            .andExpect(status().isMethodNotAllowed());
